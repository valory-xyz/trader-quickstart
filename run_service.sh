...
dotenv_set_key() {
    ...
    echo "[run_service::dotenv_set_key] Contents of $(realpath "$dotenv_path"):"
    cat "$dotenv_path"
    echo ""
...
export_dotenv() {
    dotenv_path="$(realpath "$1")"
    echo "[run_service::export_dotenv] Exporting environment variables from: $dotenv_path"
    cat $dotenv_path
    unamestr=$(uname)
    # Mac
...
create_storage() {
    ...
    echo "Your operator's private key: $operator_pkey"
    ...
    echo "Your agent instance's private key: $agent_pkey"
    echo ""
}
...
try_read_storage() {
    if [ -d $store ]; then
        ...
        echo "[run_service::try_read_storage] Contents of $env_file_path before sourcing:"
        cat "$env_file_path"
        echo ""
        echo "[run_service::try_read_storage] Sourcing $env_file_path..."
...
if [ "$rcp_exception" = true ]; then
    echo "Error: The received RCP response is malformed.  Please verify the RPCaddress and/or RCP behavior."
...
echo "[run_service::main] Contents of $(realpath "../$env_file_path"):"
cat "../$env_file_path"
...
if [ -z "$AGENT_ID" ]; then
    echo "[run_service::main] Error: AGENT_ID is not set before exporting."
fi
echo "[run_service::main] AGENT_ID is set to: $AGENT_ID"

if [ -z "$SUBGRAPH_API_KEY" ]; then
    echo "[run_service::main] Error: SUBGRAPH_API_KEY is not set before exporting."
fi
echo "[run_service::main] SUBGRAPH_API_KEY is set to: $SUBGRAPH_API_KEY"

export_dotenv "../$env_file_path"

if [ -z "$SUBGRAPH_API_KEY" ]; then
    echo "[run_service::main] Error: SUBGRAPH_API_KEY is not set. Please ensure it's properly configured in the .env file."
    exit 1
fi
echo "[run_service::main] SUBGRAPH_API_KEY is set to: $SUBGRAPH_API_KEY"

if [ -z "$AGENT_ID" ]; then
    echo "[run_service::main] Error: AGENT_ID is not set. Please ensure it's properly configured in the .env file."
    exit 1
fi
echo "[run_service::main] AGENT_ID is set to: $AGENT_ID"
...
if [ -z ${service_id+x} ]; then
    ...
    echo "[run_service::main] Service ID Path: $service_id_path"
...
echo "[run_service::main] Service Deployed!"
...
echo "[run_service::main] Performing staking operations..."
perform_staking_ops
...
then
    ...
    if ! [ -d "$service_dir" ]; then
        echo "[run_service::main] Fetching the service..."
        # Fetch the service
    ...
    cd $service_dir
    # Build the image
    echo "[run_service::main] Building the service..."
